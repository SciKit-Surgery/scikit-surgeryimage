...
class OneSourcePerFileWriter(VideoWriterBase):
    ...
    def create_video_writers(self):
        ...
        for filename, frame in zip(filenames, self.frame_source.frames):
            ...
            video_writer = cv2.VideoWriter(filename, self.fourcc, self.fps, (width, height))

            self.video_writers.append(video_writer)
            logging.debug("Created VideoWriter. Filename: {} codec: {} FPS:{} Width:{} Height:{}".format(filename, self.fourcc, self.fps, width, height))
...
class StackedVideoWriter(VideoWriterBase):
    ...
    def create_video_writers(self):
        ...
        self.video_writer = cv2.VideoWriter(self.filename, self.fourcc, self.fps, (width, height))

        LOGGER.info("Saving to : %s", self.filename)
        logging.debug("Created VideoWriter. Filename: {} codec: {} FPS:{} Width:{} Height:{}".format(self.filename, self.fourcc, self.fps, width, height))
